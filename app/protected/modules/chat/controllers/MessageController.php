	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Message']))
		{
                        if(!Yii::app()->user->isGuest){
                            $_POST['Message']['userId'] = Yii::app()->user->id;
                        } else {
			...
			if($model->save()){
				...
				$this->redirect(array('view','id'=>$model->id));
                            ...
                            $errorStr =  $model->getErrors();
                        }
		}
                if(Yii::app()->request->isAjaxRequest){
                    $output['status'] = 'Error';
                    $output['message'] = (empty($errorStr))?'Error on send':$errorStr;
                    $output['console'] = (empty($errorStr))?'Error create message':$errorStr;
                    echo CJSON::encode($output);
	...
	public function actionIndex()
	{
        ...
        public function actionGetLast($onlyAfter = 0){
            ...
            foreach ( $messageAll as $messageKey => $messageOne ) {
                ...
                if(!empty($userAll[$oneRow['userId']])){
                    $oneRow['userName'] = $userAll[$oneRow['userId']]['username'];
                }
                $output[] =  $oneRow;
            }
            $output = array('mesages'=>$output);
            echo CJSON::encode($output);
        }
